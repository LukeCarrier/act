...
func TestGetWorkflowSecrets_Masking(t *testing.T) {
	ctx := context.Background()

	// Test that environment-specific secrets are added to masks
	t.Run("environment secrets are masked", func(t *testing.T) {
		...
		assert.Contains(t, rc.Masks, "env_secret_value")
		assert.NotContains(t, rc.Masks, "repo_secret_value") // repo secrets are masked elsewhere
	})

	// Test that insecure secrets are not masked
	t.Run("insecure secrets are not masked", func(t *testing.T) {
		...
		assert.NotContains(t, rc.Masks, "env_secret_value")
	})
}
